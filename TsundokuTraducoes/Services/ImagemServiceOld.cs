...
{
    public class ImagemServiceOld : IImagemServiceOld
    {   
        public Result ProcessaUploadCapaObra(ObraDTO obraDTO)
        {
            ...
            using var fileStream = new FileStream(caminhoArquivoImagem, FileMode.Create);
            imagemCapaPrincipal.CopyTo(fileStream);
        ...
        {            
            ...
            using var fileStream = new FileStream(caminhoArquivoImagemBanner, FileMode.Create);
            imagemBanner.CopyTo(fileStream);
        ...
        public Result ProcessaUploadCapaVolume(VolumeDTO volumeDTO, string numeroVolume, string diretorioImagemObra)
        {
            ...
            if (Directory.Exists(diretorioImagemObra))
            {
                ...
                var imagemVolume = Path.Combine(diretorioImagemVolume, nomeImagemVolume);
                SalvaArquivoFormFile(imagemCapa, imagemVolume);

                volumeDTO.DiretorioImagemVolume = diretorioImagemVolume;
                volumeDTO.ImagemVolume = imagemVolume;
            }
            else
            {
