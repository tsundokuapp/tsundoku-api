...
public class AwsS3Service : IAwsS3Service
{
    ...
    public async Task<Result<ImagemDTO>> RealizaUploadImagemS3(ParametroAwsServiceDto parametrosAmazonServiceDto)
    {
        var imagemDTO = new ImagemDTO();
        var pastaCriada = false;
        var existePasta = await VerificaObjetoExistenteAwsS3(parametrosAmazonServiceDto.NomeDiretorioImagens);

        if (!existePasta)
        {
            pastaCriada = await CriarPastaS3(parametrosAmazonServiceDto.NomeDiretorioImagens);
        }
        else
        {
            pastaCriada = true;
        ...
        var stream = new MemoryStream(parametrosAmazonServiceDto.ArrayByteImagem);
        var caminhoCompletoImagem = $"{parametrosAmazonServiceDto.NomeDiretorioImagens}{parametrosAmazonServiceDto.NomeArquivoImagem}";

        var imagemUpada = await UploadImagem(stream, caminhoCompletoImagem, parametrosAmazonServiceDto.AlterarImagem);

        if (!imagemUpada)
            return Result.Fail("Problema ao realizar upload de imagem na Amazon S3");

        imagemDTO.Diretorio = parametrosAmazonServiceDto.NomeDiretorioImagens;
        imagemDTO.Url = $"{_distributionDomainName}{caminhoCompletoImagem}";
